...
type FilterCategory = "roles" | "statuses"
...
const statusOptions: User["status"][] = ["active", "inactive", "blocked"]
...
export function UserFilters({ filters, onFiltersChange, totalUsers }: UserFiltersProps) {
  ...
  const filterCategories = [
    { key: "roles" as FilterCategory, label: "Роль", icon: Shield, options: roleOptions },
    {
  ...
  const getFilteredOptions = (category: (typeof filterCategories)[0]) => {
    ...
    return category.options.filter((option) => {
      const displayValue =
        category.key === "statuses" ? statusLabels[option as User["status"]] || option : option
    ...
    <div className="space-y-4">
            ...
            <PopoverTrigger asChild>
              <Button variant="outline" className="h-10 gap-2 bg-card pr-2 flex items-center justify-start" style={{ width: '200px' }}>
                ...
                {filters.period.from || filters.period.to ? (
                  ...
                  <span className="text-muted-foreground">Период регистрации</span>
                )}
              </Button>
            </PopoverTrigger>
                      ...
                      return filteredOptions.map((option) => {
                        const displayValue =
                          category.key === "statuses"
                            ? statusLabels[option as User["status"]] || option
                            : option
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          {filters.roles.map((r) => (
            <span key={r} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-card border rounded-full text-sm">
              <Shield className="h-3.5 w-3.5 text-muted-foreground" />
              <span className="text-muted-foreground">Роль:</span> {r}
              <button onClick={() => removeFilter("roles", r)} className="ml-1 p-0.5 hover:bg-muted rounded transition-colors text-muted-foreground hover:text-foreground">
          ...
          {filters.statuses.map((s) => (
            <span key={s} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-card border rounded-full text-sm">
              ...
              <span className="text-muted-foreground">Статус:</span> {statusLabels[s]}
