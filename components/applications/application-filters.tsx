...
import type { ApplicationFilterState } from "@/types/application"
import { applicationFilterOptions, getApplicationCounts } from "@/lib/mock-applications"
...
type FilterCategory = "athletes" | "competitions" | "statuses"

export function ApplicationFilters({ filters, onFiltersChange, totalApplications }: ApplicationFiltersProps) {
  ...
  const filterCategories = [
    { key: "athletes" as FilterCategory, label: "Спортсмен", icon: User, options: applicationFilterOptions.athletes },
    { key: "competitions" as FilterCategory, label: "Соревнование", icon: Trophy, options: applicationFilterOptions.competitions },
    {
      key: "statuses" as FilterCategory,
      label: "Статус",
      icon: CheckCircle,
      options: applicationFilterOptions.statuses.map((s) => s.value),
    },
  ]
  ...
  const getFilteredOptions = (category: (typeof filterCategories)[0]) => {
    ...
    return category.options.filter((option) => {
      const displayValue =
        category.key === "statuses" ? applicationFilterOptions.statuses.find((s) => s.value === option)?.label || option : option
                      ...
                      return filteredOptions.map((option) => {
                        const displayValue =
                          category.key === "statuses"
                            ? applicationFilterOptions.statuses.find((s) => s.value === option)?.label || option
                            : option
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {filters.statuses.map((s) => (
            <span key={s} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-card border rounded-full text-sm">
              ...
              <span className="text-muted-foreground">Статус:</span> {applicationFilterOptions.statuses.find((st) => st.value === s)?.label}
